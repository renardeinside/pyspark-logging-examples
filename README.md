...
```bash
conda create -n pyspark_logging_examples python=3.9
conda activate pyspark_logging_examples
```
...
```
This command will create your personal repository under `/Repos/<username>/pyspark_logging_examples`.
3. To set up the CI/CD pipeline with the notebook, create a separate `Staging` repo:
```bash
databricks repos create --url <your repo URL> --provider <your-provider> --path /Repos/Staging/pyspark_logging_examples
```
...
- To trigger the release pipeline, get the current version from the `pyspark_logging_examples/__init__.py` file and tag the current code version:
```
